...
static bool s2mu004_check_slow_charging(struct s2mu004_charger_data *charger, int input_current)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (is_wcin_type(charger->cable_type))
			s2mu004_update_reg(charger->i2c, S2MU004_CHG_CTRL1,
				1 << SEL_PRIO_WCIN_SHIFT, SEL_PRIO_WCIN_MASK);
		else
			s2mu004_update_reg(charger->i2c, S2MU004_CHG_CTRL1,
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		{
			...
			s2mu004_check_slow_charging(charger, input_current);
...
static int s2mu004_charger_parse_dt(struct device *dev,	struct s2mu004_charger_platform_data *pdata)
{
	...
	if (!np) {
		...
		pdata->chg_freq_ctrl = of_property_read_bool(np, "battery,chg_freq_ctrl");
