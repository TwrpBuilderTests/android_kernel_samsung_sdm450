...
static void sec_bat_get_charging_current_by_siop(struct sec_battery_info *battery,
	...
	if (battery->siop_level == 3) {
		...
		if(battery->pdata->use_vbus_control)
			max_charging_current = battery->pdata->siop_charging_limit_current;
		else
			max_charging_current = battery->pdata->siop_hv_charging_limit_current;
#endif
