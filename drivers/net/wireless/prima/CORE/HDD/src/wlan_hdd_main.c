...
#if 1   
	VOS_TRACE( VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_ERROR, " ##### %s", command);   
#endif	
    ...
    */
   {
       ...
       */
       ...
       else if(strncmp(command, "SETSUSPENDMODE", 14) == 0)
       {
		   ...
		   VOS_TRACE( VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_FATAL,
                  "%s: SETSUSPENDMODE Enter %s cmd", __func__, command);
        ...
        VOS_TRACE( VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_FATAL,
                  "%s: SETSUSPENDMODE EXIT %s cmd", __func__, command);
       }
#if 1
       else if(strncmp(command, "DEBUG_DUMP", 10) == 0)
       {
		   VOS_TRACE( VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_ERROR,
                      " Received Command to DEBUG_DUMP in %s", __func__);
					  
			// The operation is not yet defined from SS.		  
       }
#endif	
...
exit:
   EXIT();
       VOS_TRACE( VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_FATAL,
                  "%s: Return %s end", __func__, command);
   if (command)
   {
       kfree(command);
   }
